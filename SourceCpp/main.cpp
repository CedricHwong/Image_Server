...
int main()
{
    ...
    mysql = picdb::MySQLInit();
    picdb::ImageTable image_table(mysql);

    signal(SIGINT, [](int)
    {
        picdb::MySQLRelease(mysql);
        exit(0);
    });
...
//            cout << f.first << ":" << f.second.filename << endl;
//        }
        ...
        // body是图片的内容
        const std::string image_body = file.content;
//        cout << file.content_type<<" "<<file.filename<<endl;
        // 3、把图片属性信息插入到数据库中
        Json::Value image;
        time_t t = time(0);
        char ch[64];
        ...
        image["size"] = (int)file.content.length();
        image["upload_time"] = ch;
        image["type"] = file.content_type;
        image["path"] = "../wwwroot/" + file.filename;
